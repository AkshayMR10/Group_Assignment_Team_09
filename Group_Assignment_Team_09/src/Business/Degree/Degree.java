...
public class Degree {
    ...
    public boolean validateCoreClasses(ArrayList<SeatAssignment> sas) {
        ...
        for (Course c : corelist) {
            if (!isCoreSatisfied(sas, c)) { // if selected core class not on the transcript return false
                return false;
            }
        }
        return true; // all core classes are on the transcript satisfying core reqs
    }
